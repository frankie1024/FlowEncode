<div align="center">
  ...
  <p>
    ...
    <img alt="Version" src="https://img.shields.io/badge/version-1.8.2-167a7f">
...
FlowEncode 是一个面向 Windows x64 的桌面端编码工作流前端，围绕 `x264`、`x265`、`SVT-AV1`、`Av1an`、`FFmpeg`、`VapourSynth`、`VSPipe` 与相关公开工具链，提供蓝光解复用、脚本编辑与预览、视频压制、音频转码、自动压制、队列执行、日志查看和模板复用能力。
...
| 模块 | 能力 |
| --- | --- |
| 工作台 | 按流程进入解复用、VapourSynth、视频压制、音频转码、自动压制、模板库和设置，并显示活跃任务状态。 |
...
| 自动压制 | 基于 `Av1an` 组织目标质量流程，支持 `VMAF`、探测次数、并行度、编码器附加参数、命令预览和实时日志。 |
| 模板库 | 保存、载入、导入、导出、置顶和覆盖 `.profile` 模板，沉淀常用参数集。 |
| 环境引导 | 检测依赖状态；可引导安装或更新公开支持的运行时、插件、编码器、`FFmpeg` 与 `Av1an`，并允许手动导入、固定或移除部分解复用器和音频工具的本地副本。 |
